...
%                  \|/
%                   V
%           Session-level folder (e.g., visit1, visit2 or session1, session2...)
%                   |
%                  \|/
%                   V
%       Condition-level folder (e.g., condition1, condition2 or block 1, block2...)
%                   |
%                  \|/
%                   V
...
% Recognize the folders (Session-level)
fileslevel1=dir(Subjectfolderpath);
Folderinfo=fileslevel1([fileslevel1.isdir]);
SessionNo={Folderinfo(3:end).name}; % For example: visit 1, visit 2, ...following visits
Rawdatapath=struct();

for i=1:length(SessionNo)
    Rawdatapath(i).EEG=fullfile(Subjectfolderpath,'\',SessionNo{i},'\EEG\');
    Rawdatapath(i).Nexus=fullfile(Subjectfolderpath,'\',SessionNo{i},'\Nexus\');
end
...
for k=1:length(Rawdatapath)
    ...
    for l=1:length(EEGfilenames)
        ...
        if nargout>1
            Experiment.(['EEG_',Subjectfoldername,'_', SessionNo{k},'_', Conditionname{l}])= EEG;
            Syncarray.([Subjectfoldername,'_', SessionNo{k},'_', Conditionname{l}])= syncdata;
        else
            Experiment.(['EEG_',Subjectfoldername,'_', SessionNo{k},'_', Conditionname{l}])= EEG;
        end
        ...
        % convert setfile to BIDs structure
        cfg = [];
        cfg.method    = 'copy';
        cfg.datatype  = 'eeg';
